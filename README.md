...
Library for HTTP e2e and scenario testing with golden files.
...
Once a golden file is generated by `go test -golden`, e2e compares the HTTP
...
Run tests with `-golden` to generate or update golden files.

```sh
go test -v ./... -golden
```

Then run tests without `-golden` to compare responses.
